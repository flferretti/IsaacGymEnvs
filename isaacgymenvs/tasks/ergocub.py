...
class ErgoCub(VecTask):
    def __init__(
        ...
        self.cfg["env"]["numObservations"] = 108 - 16
...
def compute_humanoid_reward(
    ...
    scaled_cost = (
        joints_at_limit_cost_scale * (torch.abs(obs_buf[:, 12:32]) - 0.98) / 0.02
    )
    dof_at_limit_cost = torch.sum(
        (torch.abs(obs_buf[:, 12:32]) > 0.98)
        * scaled_cost
    ...
    electricity_cost = torch.sum(
        torch.abs(actions * obs_buf[:, 32:52]) * motor_effort_ratio.unsqueeze(0), dim=-1
    )
...
def compute_humanoid_observations(
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, Tensor, float, float, float, Tensor, Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]
    ...
    # obs_buf shapes: 1, 3, 3, 1, 1, 1, 1, 1, num_dofs (20), num_dofs (20), 6, num_acts (20)
    obs = torch.cat(
        (
